...
async def start_ai_agent(
    ...
    tmux_info = ensure_tmux_split(
        config, log_path=log_path, config_path=config_path, cli_path=cli_path,
    )
    if tmux_info:
        return tmux_info

    options = AgentOptions(config_path=config_path)
    runner = AgentRunner(config=config, plugin=plugin)
    thread = threading.Thread(
        target=runner.follow,
...
async def start_ai_agent_from_context(
    ...
    if not enabled or log_path is None:
        return None
    return await start_ai_agent(
        config_path=config_path, log_path=log_path, plugin=plugin, cli_path=cli_path,
    )
