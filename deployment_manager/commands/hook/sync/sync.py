...
async def sync_hook(sync_file: Path) -> None:
    if not await sync_file.exists():
        display_warning(
            f"Sync file {sync_file} does not exist."
        )
        return
    ...
    if not parsed_url.path.endswith(".py") or not hook_to_sync["local_path"].endswith(".py"):
        display_error(f"Processing of {hook_name} error: Repo path and local path must point to a .py file")
        return
    ...
    if not remote_file:
        display_error(f"Processing of {hook_name} error: Pull from remote unsuccessful.")
        return

    remote_file = remote_file.decode("utf-8")
    local_file_path = Path(hook_to_sync["local_path"])
    if not await local_file_path.exists():
        display_error(f"Processing of {hook_name} error: Local path does not exist.")
        return
