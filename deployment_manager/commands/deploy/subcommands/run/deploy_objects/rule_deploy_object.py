...
class RuleDeployObject(DeployObject):
    ...
    async def override_references_in_target_object_data(self, data_attribute, target, use_dummy_references):
        ...
        # Structured URL replacement for action payload references.
        # Unstructured replacement only swaps IDs, leaving the source org's base URL intact
        # (e.g., a label URL keeps `source.rossum.app` after a cross-org deploy).
        self._override_action_payload_references(data=data, target=target, use_dummy_references=use_dummy_references)

    def _override_action_payload_references(self, data: dict, target, use_dummy_references: bool):
        for action in data.get("actions", []) or []:
            payload = action.get("payload") or {}
            action_type = action.get("type", "")

            if action_type in ("add_label", "add_remove_label", "remove_label"):
                self.ref_replacer.replace_list_of_reference_urls(
                    object=payload,
                    target_index=target.index,
                    target_objects_count=len(self.targets),
                    dependency_name="labels",
                    lookup_table=self.deploy_file.lookup_table,
                    reverse_lookup_table=self.deploy_file.reverse_lookup_table,
                    object_type=CustomResource.Label,
                    use_dummy_references=use_dummy_references,
                ...
                self.ref_replacer.replace_reference_url(
                    ...
                    target_index=target.index,
                    target_objects_count=len(self.targets),
                    dependency_name="email_template",
                    lookup_table=self.deploy_file.lookup_table,
                    reverse_lookup_table=self.deploy_file.reverse_lookup_table,
                    object_type=Resource.EmailTemplate,
                    use_dummy_references=use_dummy_references,
                    keep_dependency_without_equivalent=True,
                )
