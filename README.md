...
To reinstall with newer version, run `pipx install . --force`.
...
Fill in the required credentials in the `credentials.json` file that was created in the project folder - `source.api_base` (ie.: https://elis.rossum.ai/api/v1, https://mydomain.rossum.app/api/v1) and either `source.token` (retrieved using /auth endpoint) or `source.username` and `source.password`.  
