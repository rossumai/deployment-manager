...
def override_attribute(complete_mapping, mapping: dict, object: Organization | Workspace | Hook | Schema | Queue | Inbox) -> Organization | Workspace | Hook | Schema | Queue | Inbox:
    ...
    if mapping.get("attribute_override"):
        ...
        for key in keys:
            if type(mapping["attribute_override"][key]) is dict:
                path = mapping["attribute_override"][key]["path"]
                replace_reference_on_path(complete_mapping, [key] + path.split("."), mapping["attribute_override"][key]["type"], object)
            else:
                object[key] = mapping["attribute_override"][key]
    return object

def replace_reference_on_path(complete_mapping:dict, path:list, reference_type:str, object: Organization | Workspace | Hook | Schema | Queue | Inbox) -> Organization | Workspace | Hook | Schema | Queue | Inbox:
    ...
    if object and type(object) is list:
        for node in object:
            if type(node) is dict and path[0] not in node:
                replace_reference_on_path(complete_mapping, path, reference_type, node)
            else:
                new_val = replace_value_of_type(complete_mapping, reference_type, node[path[0]])
                if new_val:
                    node[path[0]] = new_val
    elif object and type(object) is dict:
...
def convert_value(value):
    ...
    if type(value) is int:
        return "int", value
    elif type(value) is str and "https" in value:
