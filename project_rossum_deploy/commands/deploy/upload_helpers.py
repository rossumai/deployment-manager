...
async def upload_workspace(
    ...
    if target_id:
        try:
            ...
            display_error(
                f'Error while updating workspaceƒ "{workspace['name']} ({workspace['id']})" -> "{target_id}:',
                e,
            )
            return {}
    else:
        try:
            ...
            print(
                f'Released (created) workspace "{workspace['id']}" -> "{result['id']}".'
            )
            return result
        except Exception as e:
            display_error(
                f'Error while creating workspace  "{workspace['name']} ( {workspace['id']})":',
                e,
            )
            return {}
...
async def upload_schema(
    ...
    if target_id:
        ...
        try:
            ...
            print(
                f'Released (updated) schema "{schema['name']} ({schema['id']})" -> "{target_id}".'
            )
            return result
        except Exception as e:
            display_error(
                f'Error while updating schema "{schema['name']} ({schema['id']})" -> "{target_id}:',
                e,
            )
            return {}
    else:
        try:
            ...
            print(
                f'Released (created) schema "{schema['name']} ({schema['id']})" -> "{result['id']}".'
            )
            return result
        except Exception as e:
            display_error(
                f'Error while creating schema  "{schema['name']} ({schema['id']})":', e
            )
            return {}
...
async def upload_hook(
    ...
    if target_id:
        try:
            ...
            display_error(
                f'Error while updating hook "{hook['name']} ({hook['id']})" -> "{target_id}:',
                e,
            )
            return {}
    else:
        try:
            ...
            display_error(
                f'Error while creating hook  "{hook['name']} ( {hook['id']})":', e
            )
            return {}
